...
完成 VA 到 PA 转换的一系列 translation table lookup 称为 translation table walk。它是由硬件自动执行的，and can have a significant cost in execution time. 为了实现从 VA 到 PA 的细粒度的映射，一次地址转换，需要访问多次 translation table，每次访问都会返回更精确的映射。
